...
// Risk overlay: rank-based bins for visualization so bin populations follow an
// exponential rule by COUNT (66% in lowest bin, then 66% of remaining, etc.).
// Underlying risk scores remain unchanged.
let riskOverlayScale = null; // { thresholds:number[], keepFraction:number, bins:number } | null
let riskOverlayLegend = null; // { labels: string[], palette: string[], thresholds: number[] }
let riskOverlayDirty = true;
let riskOverlayLastReportKey = null;
  ...
  if (legendOverlayKeyEl) {
    ...
    if (filterState.overlay !== "none") {
      if (filterState.overlay === "breaks") {
        ...
        // Roads overlay shows *just* the roads-driven LoF uplift (debug view).
        // Grey = no major-road exposure; colors map to uplift levels.
        ...
        if (filterState.overlay === "risk" && riskOverlayLegend?.labels?.length === 4) {
          for (let i = 0; i < riskOverlayLegend.labels.length; i++) {
            makeKeyItem({
              listEl: legendOverlayKeyEl,
              label: riskOverlayLegend.labels[i],
              color: riskOverlayLegend.palette[i],
            });
          }
        } else {
          const palette =
...
function summarizeProperties(properties, labels) {
  ...
  if (expRaw && typeof expRaw === "object") {
    ...
    const upliftTxt = Number.isFinite(uplift) ? ` (+${uplift} LoF)` : "";
    lines.push(`major road exposure (raw)${bmTxt}: yes${clsTxt}${ctpTxt}${distTxt}${upliftTxt}`);
    lines.push(`major road uplift eligible: ${roadEligible ? "yes" : "no"}`);
  } else {
  ...
  if (Number.isFinite(adj?._roadUpliftLof)) {
    ...
    lines.push(`major road uplift applied${bmTxt}: yes${clsTxt}${ctpTxt}${distTxt}${bumpTxt}`);
  }
...
function render(geojson, labels) {
  ...
  function updateSymbology(k) {
    ...
    // Deterministic tie-breaker for risk quantiles.
    // When many assets share the same max risk score (e.g., 16), plain quantiles
    // can collapse (q1=q2=q3), making the map look "all red". This tiny jitter
    // is for visualization only and does not affect the underlying risk scores.
    ...
    function riskScoreForOverlay(d) {
      ...
      const gid = stableIdFor(d);
      // Keep epsilon well below score step sizes.
      return base + stableHash01(gid) * 1e-4;
    }
    ...
    function recomputeRiskOverlayScaleIfNeeded() {
      if (!riskOverlayDirty) return;
      riskOverlayDirty = false;

      if (styleMode !== "risk") {
        riskOverlayScale = null;
        riskOverlayLegend = null;
        return;
      }

      // Build a sortable list of (feature, score). We bin by *rank* so counts follow
      // the exponential rule even when many features have identical risk scores.
      const items = [];
      for (const f of features) {
      ...
      for (let bin = 1; bin <= bins; bin++) {
        ...
        // Ensure we leave at least 1 element for each remaining bin.
        ...
        const end = idx + take; // exclusive
        for (let i = idx; i < end; i++) {
          const f = items[i].f;
          if (f?._derived) f._derived.riskOverlayBin = bin;
        }

        // Threshold for legend: last score in this bin (except final bin).
        if (bin < bins) {
          thresholds.push(items[end - 1].v);
        }

        idx = end;
      }

      riskOverlayScale = { thresholds, keepFraction: keep, bins };

      const palette = riskModel.riskPaletteN(bins);
      const labels = [];
      if (thresholds.length === bins - 1) {
        labels.push(`Bin 1 (66% lowest, ≤${fmt(thresholds[0])})`);
        for (let i = 1; i < thresholds.length; i++) {
          labels.push(`Bin ${i + 1} (${fmt(thresholds[i - 1])}–${fmt(thresholds[i])})`);
        }
        labels.push(`Bin ${bins} (top tail, >${fmt(thresholds[thresholds.length - 1])})`);
      }

      riskOverlayLegend = { palette, thresholds, labels };
    }

    recomputeRiskOverlayScaleIfNeeded();

    // Quantify how much is "red" (top bin) for the *currently displayed* mains.
    // This is intentionally console-based to keep UX minimal.
    if (styleMode === "risk") {
      let totalShown = 0;
      let redShown = 0;
      const bins = Array.from({ length: RISK_OVERLAY_BINS }, () => 0);

      for (const f of features) {
        if (!passesFilters(f)) continue;
        if (!passesLod(f)) continue;
        totalShown += 1;

        const derived = f?._derived;
        const bin =
          riskOverlayScale && Number.isFinite(Number(derived?.riskOverlayBin))
            ? clamp(Number(derived.riskOverlayBin), 1, RISK_OVERLAY_BINS)
            : clamp(Number(derived?.riskBin ?? 2), 1, 4);

        bins[bin - 1] += 1;
        if (bin === RISK_OVERLAY_BINS) redShown += 1;
      }

      const pctRed = totalShown > 0 ? (redShown / totalShown) * 100 : 0;
      const qKey =
        Array.isArray(riskOverlayLegend?.thresholds)
          ? riskOverlayLegend.thresholds
              .map((x) => (Number.isFinite(Number(x)) ? Number(x).toFixed(3) : "?"))
              .join(",")
          : "none";
      const reportKey = `${Math.round(k * 100) / 100}|${minDiam}|${totalShown}|${qKey}`;
      if (reportKey !== riskOverlayLastReportKey) {
        riskOverlayLastReportKey = reportKey;
        let totalAfterFilters = 0;
        for (const f of features) {
          if (passesFilters(f)) totalAfterFilters += 1;
        }

        console.info("RISK OVERLAY DISTRIBUTION (currently drawn on screen)", {
          totalInDataset: features.length,
          totalAfterFilters,
          totalDrawnNow: totalShown,
          zoomDeclutterMinDiameterMm: minDiam <= 0 ? null : minDiam,
          binCounts: bins,
          pctTopBin: Number.isFinite(pctRed) ? Number(pctRed.toFixed(1)) : null,
          thresholds: riskOverlayLegend?.thresholds ?? null,
          meaning:
            "These counts are for the mains currently being drawn (after your legend filters, plus the zoom-based decluttering that hides small diameters when zoomed out).",
        });
      }
    }
    ...
    paths
      ...
      .attr("stroke", (d) => {
        ...
        if (styleMode === "risk") {
          ...
          const bin =
            riskOverlayScale && Number.isFinite(overlayBin)
              ? clamp(overlayBin, 1, RISK_OVERLAY_BINS)
              : clamp(Number(derived?.riskBin ?? 2), 1, 4);
          const pal = riskOverlayScale ? riskModel.riskPaletteN(RISK_OVERLAY_BINS) : riskModel.riskPalette();
          return pal[clamp(bin, 1, pal.length) - 1];
        }
        ...
        if (styleMode === "risk") {
          ...
          const bin =
            riskOverlayScale && Number.isFinite(overlayBin)
              ? clamp(overlayBin, 1, RISK_OVERLAY_BINS)
              : clamp(Number(derived?.riskBin ?? 2), 1, 4);
          return lineWidthForLevel(bin, k, riskOverlayScale ? RISK_OVERLAY_BINS : 4);
        }
        if (styleMode === "consequence") {
          const cof = clamp(Number(derived?.cof ?? 2), 1, 4);
