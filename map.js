...
const breakDensityUrl = "./docs/break_density_by_material.json";
const roadProximityUrl = "./docs/road_proximity_by_main.json";
const roadsUrl = "./data/Major_Road_Network_20251231.geojson";
const coxRiskScoresUrl = "./outputs/cox/segment_hazard_scores.csv";
const coxBreakDiffUrl = "./outputs/cox/break_count_vs_expected.csv";

// Risk visualization mode:
// - rank (default): color by rank-binned risk score for visual contrast (6 bins, 90% keep rule)
// - class: color by computed RiskClass 1..4
const RISK_VIZ_MODE = (new URLSearchParams(window.location.search).get("riskViz") || "rank")
  .toString()
  .trim()
  .toLowerCase();

// Loaded from docs/break_density_by_material.json (generated by tools/break_density_by_material.py)
let breakDensityByMaterial = new Map(); // material_code -> breaks_per_km
...
// Optional risk overlay contrast mode: rank-based bins for visualization.
// Enabled only when `?riskViz=rank`.
let riskOverlayDirty = true;
let riskOverlayLegend = null; // { labels: string[], palette: string[], thresholds: number[] }

const RISK_OVERLAY_BINS = 6;
const RISK_OVERLAY_KEEP_FRACTION = 0.9; // 90% in lowest, then 90% of remaining, etc.
...
// Loaded from outputs/cox/segment_hazard_scores.csv (generated by tools/cox_ph_model.py)
let coxRiskByGlobalId = new Map();
let coxMaxRiskRank = null;
let coxConsoleCountsLogged = false;
// Cox risk rank binning legend (static thresholds by percentile)
const COX_RISK_BINS = 6;
const COX_RISK_BIN_LABELS = [
  "Bottom 50%",
  "50–75%",
  "75–90%",
  "90–95%",
...
// Loaded from outputs/cox/break_count_vs_expected.csv (generated by tools/cox_break_count_vs_expected.py)
let coxBreakDiffByGlobalId = new Map();
let coxBreakDiffScale = null; // d3.scaleQuantile returning 1..COX_BREAK_DIFF_BINS
let coxBreakDiffLegend = null; // { labels: string[], palette: string[], quantiles: number[] }
const COX_BREAK_DIFF_BINS = 5;

// Loaded from data/Major_Road_Network_20251231.geojson
let roadsGeojson = null;

const ROAD_AFFECTED_MATERIALS = new Set(["CI", "AC", "PCCP", "PCI", "CON", "DI", "PDI", "YDI", "ST", "STEEL"]);
...
function writeUrlState(transform) {
  ...
  // Persist visualization mode so the map/legend can't silently change between reloads.
  params.set("riskViz", RISK_VIZ_MODE);
...
const filterState = {
  basemapEnabled: true,
  roadsEnabled: false,
  overlay: "none", // style mode: 'none' (asset symbology) | 'risk' | 'consequence' | 'breaks' | 'roads' | 'riskrank' | 'breakdiff'
...
async function loadCoxRiskScores() {
  // Expected CSV columns include: id, hazard_score, risk_rank
  // Join key: id == mains.properties.globalid
  try {
    const res = await fetch(coxRiskScoresUrl, { cache: "no-store" });
    ...
    let validRows = 0;
    for (const r of rows) {
      const gid = (r?.id ?? "").toString().trim();
      if (!gid) continue;
      const hazardScore = Number((r?.hazard_score ?? "").toString().trim());
      const riskRank = Number((r?.risk_rank ?? "").toString().trim());
      if (!Number.isFinite(riskRank) || riskRank <= 0) continue;
      validRows += 1;
      maxRank = Math.max(maxRank, riskRank);
      map.set(gid, {
        hazardScore: Number.isFinite(hazardScore) ? hazardScore : null,
        riskRank,
      });
    }

    coxRiskByGlobalId = map;
    coxMaxRiskRank = maxRank > 0 ? maxRank : null;
    // Log a quick sanity count: how many CSV rows fall into the Top 1% bin.
    let top1pct = 0;
    if (Number.isFinite(maxRank) && maxRank > 1) {
      for (const v of coxRiskByGlobalId.values()) {
        const pHigh = 1 - (Number(v.riskRank) - 1) / (maxRank - 1);
        if (pHigh >= 0.99) top1pct += 1;
      }
    }
    console.info("Cox risk scores loaded", {
      csvRows: rows.length,
      validRows,
      uniqueIds: coxRiskByGlobalId.size,
      maxRank,
      top1pct,
    });
  } catch (err) {
    coxRiskByGlobalId = new Map();
    coxMaxRiskRank = null;
    console.warn("Cox risk scores not loaded:", err);
  }
}

async function loadCoxBreakDiff() {
  // Expected CSV columns: id, observed_breaks, expected_breaks, diff_breaks, hazard_score
  // Join key: id == mains.properties.globalid
  try {
    const res = await fetch(coxBreakDiffUrl, { cache: "no-store" });
    if (!res.ok) throw new Error(`HTTP ${res.status} ${res.statusText}`);
    const text = await res.text();
    const rows = d3.csvParse(text);
    ...
    for (const r of rows) {
      ...
      const observed = Number((r?.observed_breaks ?? "").toString().trim());
      const expected = Number((r?.expected_breaks ?? "").toString().trim());
      const diff = Number((r?.diff_breaks ?? "").toString().trim());
      const hazard = Number((r?.hazard_score ?? "").toString().trim());

      const o = Number.isFinite(observed) ? observed : null;
      const e = Number.isFinite(expected) ? expected : null;
      const d = Number.isFinite(diff) ? diff : null;
      const h = Number.isFinite(hazard) ? hazard : null;

      map.set(gid, { observed: o, expected: e, diff: d, hazard: h });
      if (d != null) diffs.push(d);
    }

    coxBreakDiffByGlobalId = map;

    if (diffs.length > 0) {
      coxBreakDiffScale = d3
        .scaleQuantile()
        .domain(diffs)
        .range(d3.range(1, COX_BREAK_DIFF_BINS + 1));

      const qs = coxBreakDiffScale.quantiles();
      const labels = [];
      for (let i = 0; i <= qs.length; i++) {
        if (i === 0) labels.push(`≤ ${formatNumber(qs[0], 2)}`);
        else if (i === qs.length) labels.push(`> ${formatNumber(qs[qs.length - 1], 2)}`);
        else labels.push(`${formatNumber(qs[i - 1], 2)} – ${formatNumber(qs[i], 2)}`);
      }

      coxBreakDiffLegend = {
        labels,
        palette: riskModel.riskPaletteN(COX_BREAK_DIFF_BINS),
        quantiles: qs,
      };
    } else {
      coxBreakDiffScale = null;
      coxBreakDiffLegend = null;
    }

    console.info("Cox break diff loaded", {
      csvRows: rows.length,
      uniqueIds: coxBreakDiffByGlobalId.size,
      finiteDiffs: diffs.length,
    });
  } catch (err) {
    coxBreakDiffByGlobalId = new Map();
    coxBreakDiffScale = null;
    coxBreakDiffLegend = null;
    console.warn("Cox break diff not loaded:", err);
  }
}

function coxPercentHighForRank(riskRank) {
  const r = Number(riskRank);
  const maxRank = Number(coxMaxRiskRank);
  if (!Number.isFinite(r) || r <= 0) return null;
  if (!Number.isFinite(maxRank) || maxRank <= 1) return null;
  // riskRank: 1 = highest hazard. Convert to percentile in [0, 1], where 1.0 is highest hazard.
  return 1 - (r - 1) / (maxRank - 1);
}

function coxRiskBinFromPercentHigh(p) {
  const x = Number(p);
...
function formatInspectorText({ props, derived, explanation, adjusted }) {
  ...
  // Cox risk rank (optional overlay output).
  {
    const rr = Number(derived?.coxRiskRank);
    const hs = Number(derived?.coxHazardScore);
    const bin = Number(derived?.coxRiskBin);
    const pHigh = Number(derived?.coxPercentHigh);
    const hasAny = Number.isFinite(rr) || Number.isFinite(hs) || Number.isFinite(bin) || Number.isFinite(pHigh);
    if (hasAny) {
      const binLabel =
        Number.isFinite(bin) && bin >= 1 && bin <= COX_RISK_BIN_LABELS.length ? COX_RISK_BIN_LABELS[bin - 1] : null;
      lines.push("Cox risk (survival model):");
      lines.push(`  risk_rank: ${Number.isFinite(rr) ? rr : "(missing)"}${binLabel ? ` (${binLabel})` : ""}`);
      lines.push(`  hazard_score: ${Number.isFinite(hs) ? formatNumber(hs, 6) : "(missing)"}`);
      lines.push(`  percentile: ${Number.isFinite(pHigh) ? `${formatNumber(pHigh * 100, 1)}% high` : "(unknown)"}`);
      lines.push("");
    }
  }

  // Cox observed vs expected break counts (optional).
  {
    const o = Number(derived?.coxObservedBreaks);
    const e = Number(derived?.coxExpectedBreaks);
    const d = Number(derived?.coxBreakDiff);
    const hasAny = Number.isFinite(o) || Number.isFinite(e) || Number.isFinite(d);
    if (hasAny) {
      lines.push("Cox breaks (observed vs expected):");
      lines.push(`  observed_breaks: ${Number.isFinite(o) ? o : "(missing)"}`);
      lines.push(`  expected_breaks: ${Number.isFinite(e) ? formatNumber(e, 2) : "(missing)"}`);
      lines.push(`  diff (obs-exp): ${Number.isFinite(d) ? formatNumber(d, 2) : "(missing)"}`);
      lines.push("");
    }
  }
...
async function loadBreakDensity() {
  // Expected format: { byMaterial: [{material, breaks_per_km, ...}, ...] }
  try {
    const res = await fetch(breakDensityUrl, { cache: "no-store" });
    if (!res.ok) throw new Error(`HTTP ${res.status} ${res.statusText}`);
    const data = await res.json();
    const rows = Array.isArray(data?.byMaterial) ? data.byMaterial : [];

    const map = new Map();
    const densities = [];
    for (const r of rows) {
      const mat = (r?.material ?? "").toString().trim() || "UNKNOWN";
    ...
    breakDensityByMaterial = new Map();
    breakDensityScale = null;
    breakDensityLegend = null;
...
function renderLegend(labels, onToggle) {
  ...
  if (legendOverlayEl) {
    ...
    if (coxRiskByGlobalId?.size) {
      const swCox = document.createElement("div");
      swCox.className = "swatch";
      swCox.style.borderTopWidth = "4px";
      swCox.style.borderTopColor = riskModel.riskPaletteN(COX_RISK_BINS)[COX_RISK_BINS - 1];
      makeCheckbox({
        listEl: legendOverlayEl,
        label: "Risk rank (Cox)",
        checked: filterState.overlay === "riskrank",
        kind: "overlay",
        value: "riskrank",
        swatchEl: swCox,
      });
    ...
    if (coxBreakDiffByGlobalId?.size) {
      const swDiff = document.createElement("div");
      swDiff.className = "swatch";
      swDiff.style.borderTopWidth = "4px";
      swDiff.style.borderTopColor = riskModel.riskPaletteN(COX_BREAK_DIFF_BINS)[COX_BREAK_DIFF_BINS - 1];
      makeCheckbox({
        listEl: legendOverlayEl,
        label: "Break diff (Cox)",
        checked: filterState.overlay === "breakdiff",
        kind: "overlay",
        value: "breakdiff",
        swatchEl: swDiff,
      });
  ...
  if (legendOverlayKeyTitleEl) {
    const showKey =
      ...
      !(filterState.overlay === "breakdiff" && !coxBreakDiffLegend);
    legendOverlayKeyTitleEl.style.display = showKey ? "block" : "none";
    if (showKey) {
      if (filterState.overlay === "risk") {
        legendOverlayKeyTitleEl.textContent = "Risk";
      } else if (filterState.overlay === "consequence") {
        legendOverlayKeyTitleEl.textContent = "Consequence";
      } else if (filterState.overlay === "roads") {
        legendOverlayKeyTitleEl.textContent = "Road exposure";
      } else if (filterState.overlay === "riskrank") {
        legendOverlayKeyTitleEl.textContent = "Risk rank (Cox)";
      } else if (filterState.overlay === "breakdiff") {
        legendOverlayKeyTitleEl.textContent = "Observed - Expected (Cox)";
      } else if (filterState.overlay === "breaks") {
        legendOverlayKeyTitleEl.textContent = "Break density";
      }
    }
  }
  ...
  if (legendOverlayKeyEl) {
    ...
    if (filterState.overlay !== "none") {
      if (filterState.overlay === "breaks") {
        ...
        const palette = riskModel.riskPaletteN(COX_RISK_BINS);
        for (let i = 0; i < COX_RISK_BIN_LABELS.length; i++) {
          makeKeyItem({
            listEl: legendOverlayKeyEl,
            label: COX_RISK_BIN_LABELS[i],
            color: palette[i],
          });
        }
      } else if (filterState.overlay === "breakdiff") {
        if (coxBreakDiffLegend) {
          for (let i = 0; i < coxBreakDiffLegend.labels.length; i++) {
            makeKeyItem({
              listEl: legendOverlayKeyEl,
              label: coxBreakDiffLegend.labels[i],
              color: coxBreakDiffLegend.palette[i],
            });
          }
        } else {
          makeKeyItem({
        ...
        if (filterState.overlay === "risk" && RISK_VIZ_MODE === "rank") {
          const palette = riskModel.riskPaletteN(RISK_OVERLAY_BINS);
          const binLabels =
            riskOverlayLegend?.labels?.length === RISK_OVERLAY_BINS
              ? riskOverlayLegend.labels
              : Array.from({ length: RISK_OVERLAY_BINS }, (_, i) => `Bin ${i + 1}`);
...
function summarizeProperties(properties, labels) {
  ...
  if (gid) {
    const row = coxRiskByGlobalId?.get?.(gid);
    if (row && Number.isFinite(Number(row.riskRank))) {
      const pHigh = coxPercentHighForRank(row.riskRank);
      const bin = coxRiskBinFromPercentHigh(pHigh);
      const binLabel = Number.isFinite(Number(bin)) ? COX_RISK_BIN_LABELS[Number(bin) - 1] : null;
      const pct = Number.isFinite(Number(pHigh)) ? `${Math.round(Number(pHigh) * 1000) / 10}%` : null;
      const extra = [binLabel, pct ? `${pct} high` : null].filter(Boolean).join(", ");
      lines.push(`cox risk rank: ${row.riskRank}${extra ? ` (${extra})` : ""}`);
    }

    const diffRow = coxBreakDiffByGlobalId?.get?.(gid);
    if (diffRow && diffRow.diff != null) {
      lines.push(
        `cox breaks (obs-exp): ${Number.isFinite(Number(diffRow.diff)) ? formatNumber(diffRow.diff, 2) : "(missing)"}`
      );
    }
    lines.push(`globalid: ${gid}`);
  }
  ...
  if (breakDensityByMaterial?.size) {
    let matKey = material;
    if (matKey === "CON") matKey = "PCCP";
    if (matKey === "COPPER") matKey = "CU";
    const v = breakDensityByMaterial.get(matKey);
    if (Number.isFinite(v)) {
      lines.push(`breaks/km (material): ${Number(v).toFixed(3)}`);
    }
  }
...
function render(geojson, labels) {
  ...
  let coxMatched = 0;
  const coxBinCounts = [0, 0, 0, 0, 0, 0]; // bins 1..6
  for (const f of features) {
    ...
    // Attach Cox risk rank (optional).
    if (globalid && coxRiskByGlobalId?.size) {
      const row = coxRiskByGlobalId.get(globalid);
      if (row) {
        coxMatched += 1;
        derived.coxHazardScore = row.hazardScore;
        derived.coxRiskRank = row.riskRank;
        const pHigh = coxPercentHighForRank(row.riskRank);
        derived.coxPercentHigh = pHigh;
        derived.coxRiskBin = coxRiskBinFromPercentHigh(pHigh);
        const bin = Number(derived.coxRiskBin);
        if (Number.isFinite(bin) && bin >= 1 && bin <= 6) coxBinCounts[bin - 1] += 1;
      } else {
        derived.coxHazardScore = null;
        derived.coxRiskRank = null;
        derived.coxPercentHigh = null;
        derived.coxRiskBin = null;
      }
    } else {
      derived.coxHazardScore = null;
      derived.coxRiskRank = null;
    ...
    // Attach Cox observed-vs-expected break diff (optional overlay output).
    if (globalid && coxBreakDiffByGlobalId?.size) {
      const row = coxBreakDiffByGlobalId.get(globalid);
      if (row) {
        derived.coxObservedBreaks = row.observed;
        derived.coxExpectedBreaks = row.expected;
        derived.coxBreakDiff = row.diff;
        if (row.diff != null && coxBreakDiffScale) {
          derived.coxBreakDiffBin = coxBreakDiffScale(row.diff);
        } else {
          derived.coxBreakDiffBin = null;
        }
      } else {
        derived.coxObservedBreaks = null;
        derived.coxExpectedBreaks = null;
        derived.coxBreakDiff = null;
        derived.coxBreakDiffBin = null;
      }
    } else {
      derived.coxObservedBreaks = null;
    ...
    // Break density is aggregated by material.
    let breaksMatKey = derived.matCode;
    if (breaksMatKey === "CON") breaksMatKey = "PCCP";
    if (breaksMatKey === "COPPER") breaksMatKey = "CU";
    const breaksPerKm = breakDensityByMaterial?.get(breaksMatKey);
  ...
  if (!coxConsoleCountsLogged && coxRiskByGlobalId?.size) {
    coxConsoleCountsLogged = true;
    console.info("Cox risk join counts", {
      mainsFeatures: features.length,
      coxIds: coxRiskByGlobalId.size,
      matchedToMains: coxMatched,
      binCounts: {
        bin1: coxBinCounts[0],
        bin2: coxBinCounts[1],
        bin3: coxBinCounts[2],
  ...
  function updateSymbology(k) {
    ...
    function isRankRiskMode() {
      return styleMode === "risk" && RISK_VIZ_MODE === "rank";
    }
    ...
    function riskScoreForOverlay(d) {
      const derived = d?._derived;
      const base = derived?.riskScoreFloat;
      if (typeof base !== "number" || !Number.isFinite(base)) return null;
      // Deterministic tie-breaker so identical scores still produce stable ranks.
      return base + stableHash01(stableIdFor(d)) * 1e-4;
    }
    ...
    function recomputeRiskOverlayBinsIfNeeded() {
      if (!isRankRiskMode()) {
        // Clear any stale bins so switching back to class mode cannot accidentally use them.
        if (riskOverlayLegend) riskOverlayLegend = null;
        return;
      }
      if (!riskOverlayDirty) return;

      let didUpdateLegend = false;

      const items = [];
      for (const f of features) {
        if (!passesFilters(f)) continue;
        const v = riskScoreForOverlay(f);
        if (typeof v === "number" && Number.isFinite(v)) items.push({ f, v });
      }

      if (items.length < 20) {
        riskOverlayLegend = null;
        for (const { f } of items) {
          if (f?._derived) f._derived.riskOverlayBin = null;
        }
        // Keep dirty so we can recompute once derived scores exist (e.g. first paint can run
        // before _derived.riskScoreFloat is populated).
        riskOverlayDirty = true;
        return;
      }
      ...
      const bins = RISK_OVERLAY_BINS;
      const keep = RISK_OVERLAY_KEEP_FRACTION;
      const thresholds = [];
      ...
      for (let bin = 1; bin <= bins; bin++) {
        ...
        const binsLeft = bins - bin + 1;
        const maxTake = remaining - (binsLeft - 1);

        let take;
        if (bin === bins) {
          take = remaining;
        } else {
          take = Math.floor(keep * remaining);
          if (take < 1) take = 1;
        ...
        for (let i = idx; i < end; i++) {
          const f = items[i].f;
          if (f?._derived) f._derived.riskOverlayBin = bin;
        }

        if (bin < bins) thresholds.push(items[end - 1].v);
        idx = end;
      }

      const palette = riskModel.riskPaletteN(bins);
      const binLabels = [];
      if (thresholds.length === bins - 1) {
        binLabels.push(`Bin 1 (${Math.round(keep * 100)}% lowest, ≤${fmt(thresholds[0])})`);
        for (let i = 1; i < thresholds.length; i++) {
          binLabels.push(`Bin ${i + 1} (${fmt(thresholds[i - 1])}–${fmt(thresholds[i])})`);
        }
        binLabels.push(`Bin ${bins} (top tail, >${fmt(thresholds[thresholds.length - 1])})`);
      } else {
        // Fallback (should be rare): still show 6 swatches.
        for (let i = 1; i <= bins; i++) binLabels.push(`Bin ${i}`);
      }

      riskOverlayLegend = { palette, thresholds, labels: binLabels };

      // Mark clean only after we've built a usable legend.
      riskOverlayDirty = false;
      didUpdateLegend = binLabels.length === bins;

      // If we're currently showing the risk overlay, refresh the legend once bins are ready.
      if (didUpdateLegend && filterState.overlay === "risk") {
        renderLegend(labels, onToggle);
      }
    }

    recomputeRiskOverlayBinsIfNeeded();
    ...
    paths
      ...
      .attr("stroke", (d) => {
        const derived = d?._derived;
        if (styleMode === "risk") {
          if (RISK_VIZ_MODE === "rank") {
            const overlayBin = Number(derived?.riskOverlayBin);
            const b = Number.isFinite(overlayBin) ? clamp(overlayBin, 1, RISK_OVERLAY_BINS) : 1;
            return riskModel.riskPaletteN(RISK_OVERLAY_BINS)[b - 1];
          }
        ...
        if (styleMode === "riskrank") {
          const bin = Number(derived?.coxRiskBin);
          if (Number.isFinite(bin)) return riskModel.riskPaletteN(COX_RISK_BINS)[clamp(bin, 1, COX_RISK_BINS) - 1];
          return "#9ca3af";
        }

        if (styleMode === "breakdiff") {
          const bin = Number(derived?.coxBreakDiffBin);
          if (Number.isFinite(bin)) {
            return riskModel.riskPaletteN(COX_BREAK_DIFF_BINS)[clamp(bin, 1, COX_BREAK_DIFF_BINS) - 1];
          }
          return "#9ca3af";
        }
      ...
      .attr("stroke-width", (d) => {
        ...
        if (styleMode === "risk") {
          if (RISK_VIZ_MODE === "rank") {
            const overlayBin = Number(derived?.riskOverlayBin);
            const b = Number.isFinite(overlayBin) ? clamp(overlayBin, 1, RISK_OVERLAY_BINS) : 1;
            return lineWidthForLevel(b, k, RISK_OVERLAY_BINS);
          }
        ...
        if (styleMode === "riskrank") {
          const bin = Number(derived?.coxRiskBin);
          const lvl = Number.isFinite(bin) ? clamp(bin, 1, COX_RISK_BINS) : 1;
          return lineWidthForLevel(lvl, k, COX_RISK_BINS);
        }
        const diamMm = derived?.diamMm ?? parseDiameterMm(d?.properties?.diam);
        return strokeWidthPx(diamMm, k);
      })
