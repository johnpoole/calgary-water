...
Break points do not have material, so we use the precomputed break→main link table
to inherit main attributes.

Important:
- This tool counts one break for *each* linked pipe segment row.
- If a break point is within the radius of multiple pipes (e.g., intersections), it
    will be counted multiple times across those materials. This is intentional.
...
Notes:
- Uses `properties.length` (meters) on mains for the denominator.
- Numerator comes from outputs/breaks/breaks_to_mains_within_2m.csv, generated by:
...
# Link table: one row per break, matched to the largest-diameter pipe within 3m.
LINKS_CSV_DEFAULT = REPO_ROOT / "outputs" / "breaks" / "breaks_to_mains_largest_within_3m.csv"
...
def load_links_csv(path: Path) -> List[Dict[str, str]]:
    if not path.exists():
        raise SystemExit(
            f"Missing links CSV: {path}\n"
            "Generate it with: python tools/link_breaks_to_mains.py --within-m 2"
        )
...
def main() -> None:
    ...
    # Denominator: total pipe length by material.
    _, _, _, pipe_len_by_material_m = load_mains(MAINS_PATH)

    # Numerator: one count per (break, main) link row.
    links = load_links_csv(LINKS_CSV_DEFAULT)
    ...
    for row in links:
        material = normalize_material(row.get("main_material"))
        break_type = _safe_str(row.get("break_type")).strip()
        break_date = _safe_str(row.get("break_date")).strip()
        main_globalid = _safe_str(row.get("main_globalid")).strip()
        break_index = _safe_str(row.get("break_index")).strip()
        distance_m = _to_float(row.get("distance_m"), default=float("nan"))
        ...
        debug_rows.append(
            {
                ...
                "main_globalid": main_globalid,
                "material": material,
                "distance_m": (round(distance_m, 3) if math.isfinite(distance_m) else ""),
            }
        )
