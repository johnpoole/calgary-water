...
// Running NET volume held in SR1 at each aligned point up to the cutoff. Above the
// trigger the reservoir FILLS (positive part of (no-diversion flow - actual));
// below the trigger it DRAINS as SR1 releases (negative part). Floored at zero so
// the model can never release more than it stored.
function cumulativeVolumeSeries(aligned, offset, cutoffMs, capacityM3) {
  const rateOf = (row) => {
  ...
  const series = [];
  let volumeM3 = 0;
  series.push(volumePoint(usable[0].ms, 0, capacityM3));
  for (let index = 1; index < usable.length; index += 1) {
    ...
    series.push(volumePoint(current.ms, volumeM3, capacityM3));
  }
  return series;
}
...
export function computeSr1Model(readings, nowMs, capacityM3) {
  ...
  const diversion = aligned
    .filter((row) => row.ms <= cutoffMs)
    .map((row) => ({ ms: row.ms, value: (row.projected + offset) - row.actual }));

  const volumeSeries = cumulativeVolumeSeries(aligned, offset, cutoffMs, capacityM3);
